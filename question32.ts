...
let current_users=["Shakeel","Ahmad","Naeem","Ali","Bilal"];
let new_users=["Ahmad","Bilal","Syed","Khan","Jan"];
 
  function checkUsersNameAvailability(username:string):boolean{
    return current_users.includes(username)
  }
new_users.forEach((username)=>{
    if(checkUsersNameAvailability(username)){
        console.log(`The username  ${username} already exists, kindly enter a new username`)
    }else{
